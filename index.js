...
function getHookDeps(hookNode) {
  ...
  if (!(hookNode.arguments.length == 2 && hookNode.arguments[1] && hookNode.arguments[1].type === 'ArrayExpression')) {
    return false;
  }
...
function isFuncDepthSameAsRoot(node) {
  var _parentFunc$parent, _parentFunc$parent2, _parentFunc$parent2$p, _parentFunc$parent3, _parentFunc$parent3$p, _parentFunc$parent3$p2, _parentFunc$parent4, _parentFunc$parent4$p, _parentFunc$parent4$p2;
  ...
  var isParentVDeclarationGroup = (parentFunc == null ? void 0 : (_parentFunc$parent = parentFunc.parent) == null ? void 0 : _parentFunc$parent.type) === 'VariableDeclarator' && (parentFunc == null ? void 0 : (_parentFunc$parent2 = parentFunc.parent) == null ? void 0 : (_parentFunc$parent2$p = _parentFunc$parent2.parent) == null ? void 0 : _parentFunc$parent2$p.type) === 'VariableDeclaration';

  if (isParentVDeclarationGroup && ((parentFunc == null ? void 0 : (_parentFunc$parent3 = parentFunc.parent) == null ? void 0 : (_parentFunc$parent3$p = _parentFunc$parent3.parent) == null ? void 0 : (_parentFunc$parent3$p2 = _parentFunc$parent3$p.parent) == null ? void 0 : _parentFunc$parent3$p2.type) === 'Program' || exportDeclarations.indexOf(parentFunc == null ? void 0 : (_parentFunc$parent4 = parentFunc.parent) == null ? void 0 : (_parentFunc$parent4$p = _parentFunc$parent4.parent) == null ? void 0 : (_parentFunc$parent4$p2 = _parentFunc$parent4$p.parent) == null ? void 0 : _parentFunc$parent4$p2.type) > -1)) {
    return true;
  }
...
function isInCustomHookDef(node) {
  ...
  var normalFuncDef = getParentOfNodeType(returnInBlock, 'ArrowFunctionExpression');
  ...
  var varDeclaratorOfFunc = getParentOfNodeType(nearestFuncDef, 'VariableDeclarator');
  var varDefOfFunc = getParentOfNodeType(varDeclaratorOfFunc, 'VariableDeclaration');
  var varDefOnRoot = ((_varDefOfFunc$parent = varDefOfFunc.parent) == null ? void 0 : _varDefOfFunc$parent.type) === 'Program' || false;
  return varDefOnRoot && ((_varDeclaratorOfFunc$ = varDeclaratorOfFunc.id) == null ? void 0 : _varDeclaratorOfFunc$.type) === 'Identifier' && ((_varDeclaratorOfFunc$2 = varDeclaratorOfFunc.id) == null ? void 0 : _varDeclaratorOfFunc$2.name.startsWith('use'));
}
...
function isInRefOrMemo(node) {
  ...
  if (node.type === 'CallExpression') {
    ...
    if (callee.type === 'MemberExpression' && callee.object.name === 'React' && validCalleeNames.indexOf(callee.property.name) > -1) {
      return true;
    }
  }
}
...
function isFunctionHookOrComponent(node) {
  if (node.type === 'ArrowFunctionExpression') {
    ...
    var name = (varDef == null ? void 0 : (_varDef$id = varDef.id) == null ? void 0 : _varDef$id.name) || '';
...
var PROXY_RENDER_PHASE_MESSAGE = 'Using proxies in the render phase would cause unexpected problems.';
var SNAPSHOT_CALLBACK_MESSAGE = 'Better to just use proxy state.';
var UNEXPECTED_STATE_MUTATING = "Mutating a proxy object itself. this might not be expected as it's not reactive.";
var COMPUTED_DECLARATION_ORDER = 'Not found, If a computed field deriving value is created from another computed, the computed source should be declared first.';
var StateSnapshot = {
  meta: {
    type: 'problem',
    docs: {
  ...
  create: function create(context) {
    ...
    var getScope = 'getScope' in context ? context.getScope.bind(context) : sourceCode.getScope.bind(sourceCode);
    return {
      Identifier: function Identifier(node) {
        var scope = getScope(node);

        if (isInComputed(node) && isInProperty(node)) {
          if (isInMemberExpression(node) && returnFirstObjectIdentifier(outerMemberExpression(node)) === node.name && isComputedIdentifier(node, scope) && returnComputedValues(node)[0].includes(returnSecondObjectIdentifier(outerMemberExpression(node))) && !returnComputedValues(node)[1].includes(returnSecondObjectIdentifier(outerMemberExpression(node)))) {
            return context.report({
              node: outerMemberExpression(node),
              message: COMPUTED_DECLARATION_ORDER
            });
          } else if (isInObjectPattern(node) && isInParams(node) && node.parent.key === node && !returnComputedValues(node)[1].includes(node.name) && returnComputedValues(node)[0].includes(node.name)) {
            return context.report({
              node: node,
              message: COMPUTED_DECLARATION_ORDER
            });
        ...
        if (isInAssignmentExpression(node) && !isInMemberExpression(node) && isUsedInUseProxy(node, scope) || isInAssignmentExpression(node) && isInMemberExpression(node) && (outerMemberExpression(node).property === node && isUsedInUseProxy(outerMemberExpression(node), scope) || node.parent.object === node && isLiteral(node) && isUsedInUseProxy(outerMemberExpression(node), scope))) {
          return context.report({
        ...
        if (kind === 'snapshot') {
          if (isReadOnly(node)) {
            if (isInCallback(node) && !isInJSXContainer(node) && !isInReactHooks(node) && !isInCustomHookDef(node)) {
              return context.report({
                node: node,
            ...
            if (isFuncDepthSameAsRoot(node) || isInJSXContainer(node) || isInCustomHookDef(node)) {
              return;
            }
          } else {
            return context.report({
...
function isInComputed(node) {
  if (node.parent && node.parent.type === 'CallExpression' && node.parent.callee.name === 'proxyWithComputed' && node.parent.arguments[1] === node) {
    return true;
  } else if (node.parent) {
...
function returnComputedValues(node) {
  if (node.parent.parent && node.parent.parent.type === 'CallExpression' && node.parent.parent.callee.name === 'proxyWithComputed' && node.parent.parent.arguments[1] === node.parent) {
    return [node.parent.properties.map(function (v) {
      return v.key.name;
    }), node.parent.properties.slice(0, node.parent.properties.indexOf(node) + 1).map(function (v) {
      return v.key.name;
    })];
...
function returnSecondObjectIdentifier(node) {
  if (node && node.object.type === 'Identifier' && node.property.type === 'Identifier') {
    return node.property.name;
  } else if (node && node.object.type === 'MemberExpression' && node.property.type === 'Identifier') {
...
function isInAssignmentExpression(node) {
  return isInSomething(node, 'AssignmentExpression') || isInSomething(node, 'UpdateExpression');
}
...
function which(name, scope) {
  ...
  scope.variables.forEach(function (variable) {
    ...
    if (init.type === 'Identifier') {
      ...
      return kind = 'state';
    } else if (init.type === 'CallExpression' && init.callee.name === 'useSnapshot') {
...
function isSameMemmberExpression(first, second) {
  ...
  if (first.property.name === second.property.name || first.property.value === second.property.value) {
    if (first.object._babelType === 'MemberExpression' && second.object._babelType === 'MemberExpression' || first.object.type === 'MemberExpression' && second.object.type === 'MemberExpression') {
      return isSameMemmberExpression(first.object, second.object);
    } else if (first.object.type === 'Identifier' && second.object.type === 'Identifier') {
...
function isUsedInUseProxy(node, scope) {
  ...
  scope.variables.forEach(function (variable) {
    ...
    if (init.parent._babelType === 'CallExpression' && init.parent.callee.name === 'useSnapshot' || init._babelType === 'CallExpression' && init.callee.name === 'useSnapshot' || init.parent.type === 'CallExpression' && init.parent.callee.name === 'useSnapshot' || init.type === 'CallExpression' && init.callee.name === 'useSnapshot') {
      if (init.arguments.length > 0 && init.arguments[0] && init.arguments[0]._babelType === 'MemberExpression' && node._babelType === 'MemberExpression' || init.arguments[0] && init.arguments[0].type === 'MemberExpression' && node.type === 'MemberExpression') {
        return isUsed = isSameMemmberExpression(node, init.arguments[0]);
      } else if (init.arguments[0].type === 'Identifier' && node.type === 'Identifier' && !isInMemberExpression(node)) {
...
function isInCallback(node) {
  ...
  if (callExpressions.includes(node.parent.type) && functionTypes.includes(node.type) || ['VariableDeclarator'].includes(node.parent.type) && functionTypes.includes(node.type)) {
...
function isInRender(node) {
  if (!node.parent || !node.parent.type) return false;
  var nearestCallbackNode = getParentOfNodeType(node, 'ArrowFunctionExpression') || getParentOfNodeType(node, 'FunctionExpression');
...
function isInUseMemo(node) {
  ...
  while (current && current.parent) {
    ...
    if (current.type === 'CallExpression' && current.callee && current.callee.name === 'useMemo') {
...
function isPassedToUseSnapshot(node) {
  ...
  while (current) {
    if (current.type === 'CallExpression' && current.callee && current.callee.name === 'useSnapshot') {
...
function isLiteral(node) {
  ...
  return memberExpression.property.type === 'Literal' || memberExpression.property.type === 'NumericLiteral';
}
  ...
  create: function create(context) {
    ...
    return {
      Identifier: function Identifier(node) {
        ...
        if (cachedIdentifier && cachedIdentifier.hasThis && isCalledByProxy(node)) {
          context.report({
...
function getIdentifier(allIdentifiers, identifier) {
  for (var i = 0; i < allIdentifiers.length; i++) {
    if (allIdentifiers[i] && allIdentifiers[i].identifier && allIdentifiers[i].identifier.name === identifier) {
